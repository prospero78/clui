...
func (b *BarChart) drawBars() {
	...
	for idx, d := range b.data {
		...
		if b.onDrawCell == nil {
			SetTextColor(fColor)
			SetBackColor(bColor)
			FillRect(b.x.Get()+types.ACoordX(pos), b.y+h-barH, barW, barH, ch)
		} else {
			...
			for dy := 0; dy < barH; dy++ {
				...
				SetBackColor(req.Bg)
				for dx := 0; dx < barW; dx++ {
					PutChar(b.x.Get()+types.ACoordX(pos+dx), b.y+h-1-dy, req.Ch)
				}
			}
		}
		...
		if b.showTitles {
			...
			if b.showMarks {
				c := parts[7]
				PutChar(b.x.Get()+types.ACoordX(pos+barW/2), b.y+h, c)
			}
			var s string
			shift := 0
			...
			DrawRawText(b.x.Get()+types.ACoordX(pos+shift), b.y+h+1, s)
		}
...
func (b *BarChart) drawLegend() {
	...
	for idx, d := range b.data {
		...
		SetTextColor(d.Fg)
		SetBackColor(d.Bg)
		PutChar(b.x.Get()+types.ACoordX(pos+width), b.y+idx, c)
		s := CutText(fmt.Sprintf(" - %v", d.Title), int(b.LegendWidth()))
		SetTextColor(fg)
		SetBackColor(bg)
		DrawRawText(b.x.Get()+types.ACoordX(pos+width+1), b.y+idx, s)
	}
}
...
func (b *BarChart) drawRulers() {
	...
	if pos > 0 {
		for dy := 0; dy < h; dy++ {
			PutChar(b.x.Get()+types.ACoordX(pos), b.y+dy, cV)
		}
	}
	if b.showTitles {
		for dx := 0; dx < vWidth; dx++ {
			PutChar(b.x.Get()+types.ACoordX(pos+dx), b.y+h, cH)
		}
	}
	if pos > 0 && b.showTitles {
		PutChar(b.x.Get()+types.ACoordX(pos), b.y+h, cC)
	}
}
