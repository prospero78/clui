...
func (l *TextDisplay) processMouseClick(ev Event) bool {
	...
	if int(dy) < int(l.height.Get())/2 {
		l.moveUp(int(ww.Get()) - 1)
	} else {
		l.moveDown(int(ww.Get()) - 1)
	}
...
func (l *TextDisplay) ProcessEvent(event Event) bool {
	...
	case EventKey:
		...
		case term.KeyArrowDown:
			...
			return true
		case term.KeyPgup:
			l.moveUp(int(l.height.Get()) - 1)
			return true
		case term.KeyPgdn, term.KeySpace:
			l.moveDown(int(l.height.Get()) - 1)
			return true
		}
